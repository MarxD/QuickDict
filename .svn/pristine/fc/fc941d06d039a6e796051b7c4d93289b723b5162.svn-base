...
/**
 * 翻译工具类,继承自异步任务类,构建需要传入TextView列表，执行Execute时传入查询字符串，返回Word实体列表 GetTransUtil
 * gettransutil = new GetTransUtil(List<Textview> list)
 * 
 * @author Marx
 * 
 */
public class GetTransBiz extends AsyncTask<String, Integer, List<Word>> {
	...
	List<TextView> views;
	Context mContext;
	public GetTransBiz(List<TextView> list,Context c) {
		this.views = list;
		mContext=c;
	}
	...
	protected List<Word> doInBackground(String... Source) {
		...
		try {
			fromyoudao = JsonParserUtil.getyoudaotranslation(HttpUtil
					.doget(sourceword));
			// 有道翻译，get方式
			Log.i("tagg", "返回对象" + fromyoudao.getsourceword() + "="
					+ fromyoudao.gettranslation());
		...
		String fucking_key = Const.APPID_BAIDU + sourceword + salt
				+ Const.BAIDU_KEY;
		// APPID+查询词+随机数+申请所得的密匙（KEY），拼接生成字符串

		try {
			params.add(new BasicNameValuePair("sign", MD5Util
					.Bit32(fucking_key)));
		...
		try {
			...
			frombaidu = JsonParserUtil.getbaidutranslation(HttpUtil.doPost(
					params, Const.BASE_URL_BAIDU));
		...
		Word fromlocal = new Word();
		try {
			new LocalDictUtil(mContext).query(sourceword, fromlocal);
		} catch (Exception e) {
			Log.i("errorsss", e.toString());
	...
	protected void onPostExecute(List<Word> result) {
		...
		for (int i = 0; i < result.size(); i++) {
			Log.i("tagg", result.get(i).getsourceword() + "="
					+ result.get(i).gettranslation());
